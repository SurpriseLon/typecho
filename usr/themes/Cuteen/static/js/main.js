...
class App {
    ...
    AjaxGetCommentInfo() {
        ...
        input.onblur = function () {
            ...
            if (!QQ || QQ.toString().length < 5) return;
            Qmsg.info("获取信息中…………")
            fetch(CuteenConfig.index + '/action/cuteen', {
                method: 'POST',
                headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'},
                body: JSON.stringify({type: "getAvatar", qq: QQ})
            }).then(res => res.json()).then(res => {
                document.getElementById('comment-author-avatar').setAttribute('src', res.data.qq_avatar);
